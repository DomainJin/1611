...
const IPAddress resolume_ip(192, 168, 0, 202);
...
void setup() {
  ...
  Serial.begin(115200);
  SerialPIC.begin(9600, SERIAL_8N1, 26, 33);
...
void loop() {
  ...
  if (packetSize) {
    ...
    if (strncmp(incomingPacket, "DIR:", 4) == 0) {
    ...
    // Kiểm tra lệnh thiết lập ngưỡng
      ...
      // Gửi giá trị ngưỡng xuống PIC qua UART
  ...
  if(waitingMainEffect) {
    ...
    Serial.println("[OSC->Resolume] Main effect finished, ready for new touch.");
    sendResolumeInitOSC(5);
    touchStartMillis = 0;
    waitingMainEffect = false;
    mainEffectStarted = false;  
    effectEnable = true;
    isTouched = false;          // <-- RESET lại trạng thái chạm
    sentOSCOnce = false;        // <-- RESET lại gửi osc
    latestStatus = -1;
    latestValue = -1;
  }
